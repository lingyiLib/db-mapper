...
use lingyiLib\DbMapper\Mapper\Valitron\Validator;
...
class Mapper implements MapperInterface
{
    ...
    protected $_collectionClass = '\\lingyiLib\\DbMapper\Mapper\\Entity\\Collection';
    protected $_queryClass = '\\lingyiLib\\DbMapper\Mapper\\Query';
     ...
     *
     * @return \lingyiLib\DbMapper\Mapper\Config
     */
     ...
     *
     * @return \lingyiLib\DbMapper\Mapper\Entity\Manager
     */
     ...
     *
     * @return \lingyiLib\DbMapper\Mapper\EventEmitter
     */
    ...
    public function hasMany(EntityInterface $entity, $entityName, $foreignKey, $localValue = null)
    {
        ...
        if (!is_subclass_of($entityName, 'lingyiLib\DbMapper\Mapper\EntityInterface')) {
            throw new \InvalidArgumentException("Related entity name must be a "
                . "valid entity that extends lingyiLib\DbMapper\Mapper\Entity. Given '" .  $entityName . "'.");
        }
    ...
    public function hasManyThrough(EntityInterface $entity, $hasManyEntity, $throughEntity, $selectField, $whereField)
    {
        ...
        if (!is_subclass_of($hasManyEntity, 'lingyiLib\DbMapper\Mapper\EntityInterface')) {
            throw new \InvalidArgumentException("Related entity name must be a "
                . "valid entity that extends lingyiLib\DbMapper\Mapper\Entity. Given '" .  $hasManyEntity . "'.");
        }

        if (!is_subclass_of($throughEntity, 'lingyiLib\DbMapper\Mapper\EntityInterface')) {
            throw new \InvalidArgumentException("Related entity name must be a "
                . "valid entity that extends lingyiLib\DbMapper\Mapper\Entity. Given '" .  $throughEntity . "'.");
        }
    ...
    public function hasOne(EntityInterface $entity, $foreignEntity, $foreignKey)
    {
        ...
        if (!is_subclass_of($foreignEntity, 'lingyiLib\DbMapper\Mapper\EntityInterface')) {
            throw new \InvalidArgumentException("Related entity name must be a "
                . "valid entity that extends lingyiLib\DbMapper\Mapper\Entity. Given '" .  $foreignEntity . "'.");
        }
     ...
     * lingyiLib\DbMapper\Mapper\Query object as a sort of in-context dynamic query method
     *
    ...
    public function connection($connectionName = null)
    {
        ...
        if (empty($connectionName)) {
            return $this->locator->getConnection($connectionName);
        } elseif ($connection =  ApplicationContext::getContainer()->get(ConnectionResolver::class)->connection($connectionName)) {
            return $connection;
        } else {
            if(class_exists($connectionName)){
                $ref = new \ReflectionClass($connectionName);
                $connectionName = $ref->getStaticPropertyValue('connection');
                return ApplicationContext::getContainer()->get(ConnectionResolver::class)->connection($connectionName);
            }
        }

        throw new Exception("Connection '" . $connectionName . "' does not exist. Please setup connection using lingyiLib\DbMapper\Mapper\Config::addConnection().");
    }
    ...
    /**
     * Create collection from lingyiLib\DbMapper\Mapper\Query object
     */
    ...
    protected function with($collection, $entityName, $with = [])
    {
        ...
        foreach ($with as $relation) {
            ...
            if (!($singleEntity instanceof Entity)) {
                throw new Exception("Relation object must be instance of 'lingyiLib\DbMapper\Mapper\Entity', given '" . get_class($singleEntity) . "'");
            }

            $fields = strrev(strchr(strrev($relation),':',true));
            $fields = $fields ? explode(',',$fields) : '*';
            $relationName = strchr($relation,':',true);
            ...
            if (!($relationObject instanceof Relation\RelationAbstract)) {
                throw new Exception("Relation object must be instance of 'lingyiLib\DbMapper\Mapper\Relation\RelationAbstract', given '" . get_class($relationObject) . "'");
            }
            ...
            $collection = $relationObject->eagerLoadOnCollection($relationName, $fields,$collection);
        }
     ...
     *
     * @param \lingyiLib\DbMapper\Mapper\Entity $entity Entity object
     ...
     *
     * @param \lingyiLib\DbMapper\Mapper\EntityInterface $entity
     * @param array $options
     * @return mixed
     ...
     * @param \lingyiLib\DbMapper\Mapper\EntityInterface $relatedEntity
     * @param \lingyiLib\DbMapper\Mapper\EntityInterface $entity
     * @param \lingyiLib\DbMapper\Mapper\Relation\RelationAbstract $relation
     * @return array Related entity errors
     */
    protected function validateRelatedEntity(EntityInterface $relatedEntity, EntityInterface $entity, \lingyiLib\DbMapper\Mapper\Relation\RelationAbstract $relation)
    {
